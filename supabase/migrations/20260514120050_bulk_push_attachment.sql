-- Optional image attachment on a bulk push. The path is stored on the
-- bulk_pushes row and copied onto every delivered message by the
-- fan-out, so each recipient sees it like any message attachment.

alter table public.bulk_pushes add column attachment_path text;

-- Re-create fire_bulk_push so each delivered message carries the push's
-- attachment (when set). Body unchanged otherwise.
...
begin
  ...
  v_attachments := case
    when v_push.attachment_path is not null
      then jsonb_build_array(v_push.attachment_path)
    else '[]'::jsonb
  end;
...
-- Bulk-push attachments aren't stored under a thread-id folder, so the
-- folder-based patient read policy doesn't cover them. Allow a patient to
-- read any message-attachment that's referenced by a message in one of
-- their own threads — which covers per-thread attachments too.
create policy "patient_read_referenced_attachments" on storage.objects
  ...
  using (
    ...
    and exists (
      ...
      where t.patient_id = auth.uid()
        and m.attachments ? storage.objects.name
    )
  );
